...
Result Btree::updateExistingInode(const Inode &inode){
	...
	if(pos == node->raw.num_keys){
		PAFFS_DBG(PAFFS_TRACE_BUG, "Tried to update existing Inode %u,"
				"but could not find it!", inode.no);
...
Result Btree::redistribute_nodes(TreeCacheNode &n, TreeCacheNode &neighbor,
	...
	if (neighbor_index != -1) {
		...
		if (!n.raw.is_leaf) {
			n.pointers[0] = neighbor.pointers[neighbor.raw.num_keys];	//getTreeNodeIndex not needed, nullptr is also allowed
	...
	while(!queue_empty(q)){
		...
		for(int i = 0; i <= n->raw.num_keys; i++){
			if(!n->raw.is_leaf){
				...
				TreeCacheNode *n_cache = nullptr;		//cache version of the copy of the former cache entry...
