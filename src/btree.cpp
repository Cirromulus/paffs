...
Btree::updateExistingInode(const Inode& inode)
{
    ...
    if (pos == node->raw.num_keys)
    {
        PAFFS_DBG(PAFFS_TRACE_BUG,
                  "Tried to update existing Inode %u,"
 ...
 */
int
Btree::length_to_root(TreeCacheNode& child)
{
 ...
 */
Result
Btree::find_branch(TreeCacheNode& target, TreeCacheNode*& outtreeCacheNode)
{
    int i = 0;
    TreeCacheNode* c = nullptr;
 ...
 */
Result
Btree::find_leaf(InodeNo key, TreeCacheNode*& outtreeCacheNode)
{
    int i = 0;
    TreeCacheNode* c = nullptr;
    ...
    while (!c->raw.is_leaf)
    {
        ...
        {  //-1 because one node is needed for insert functions.
            PAFFS_DBG(PAFFS_TRACE_ERROR,
                      "Cache size (%d) too small for depth %d!",
                      treeNodeCacheSize,
                      depth);
...
Result
Btree::find_in_leaf(TreeCacheNode& leaf, InodeNo key, Inode& outInode)
{
    unsigned int i;
 ...
 */
int
Btree::get_left_index(TreeCacheNode& parent, TreeCacheNode& left)
{
    int left_index = 0;
 ...
 */
Result
Btree::insert_into_leaf(TreeCacheNode& leaf, const Inode& newInode)
{
    int i, insertion_point;

    insertion_point = 0;
    while (insertion_point < leaf.raw.num_keys
 ...
 */
Result
Btree::insert_into_leaf_after_splitting(TreeCacheNode& leaf, const Inode& newInode)
{
 ...
 */
Result
Btree::insert_into_node(TreeCacheNode& node, int left_index, InodeNo key, TreeCacheNode& right)
{
    int i;
 ...
 */
Result
Btree::insert_into_node_after_splitting(TreeCacheNode& old_node,
                                        unsigned int left_index,
                                        InodeNo key,
                                        TreeCacheNode& right)
{
    PAFFS_DBG_S(PAFFS_TRACE_TREE,
                "Insert into node after splitting at key %u, index %d",
                key,
                left_index);
 ...
 */
Result
Btree::insert_into_parent(TreeCacheNode& left, InodeNo key, TreeCacheNode& right)
{
    int left_index;
 ...
 */
Result
Btree::insert_into_new_root(TreeCacheNode& left, InodeNo key, TreeCacheNode& right)
{
 ...
 */
int
Btree::get_neighbor_index(TreeCacheNode& n)
{
    int i;
 ...
 */
Result
Btree::remove_entry_from_node(TreeCacheNode& n, InodeNo key)
{
    unsigned int i = 0;

    // Remove the key and shift other keys accordingly.
    while (n.raw.as.branch.keys[i] != key
           && i < n.raw.num_keys)  // as.branch is OK, because it is same memory as as.leaf
        i++;
...
Result
Btree::adjust_root(TreeCacheNode& root)
{
    /* Case: nonempty root.
 ...
 */
Result
Btree::coalesce_nodes(TreeCacheNode& n,
 ...
 */
Result
Btree::redistribute_nodes(TreeCacheNode& n,
                          TreeCacheNode& neighbor,
                          int neighbor_index,
                          ...
                          unsigned int k_prime)
{
    ...
    if (neighbor_index != -1)
    {
        ...
        if (!n.raw.is_leaf)
        {
            n.pointers[0] = neighbor.pointers[neighbor.raw.num_keys];  // getTreeNodeIndex not
            ...
            neighbor.pointers[neighbor.raw.num_keys] = nullptr;
            ...
            n.parent->raw.as.branch.keys[k_prime_index] =
                    neighbor.raw.as.branch.keys[neighbor.raw.num_keys - 1];
        }
        else
        {
 ...
 */
Result
Btree::delete_entry(TreeCacheNode& n, InodeNo key)
{
    int min_keys;
 ...
 */
void
Btree::print_leaves(TreeCacheNode& c)
{
    if (c.raw.is_leaf)
    {
        printf("| ");
 ...
 */
void
Btree::print_queued_keys_r(queue_s* q)
{
        ...
        for (int i = 0; i <= n->raw.num_keys; i++)
        {
            if (!n->raw.is_leaf)
            {
                ...
                TreeCacheNode* n_cache =
                        nullptr;  // cache version of the copy of the former cache entry...
...
void
Btree::print_keys(TreeCacheNode& c)
{
    queue_s* q = queue_new();
