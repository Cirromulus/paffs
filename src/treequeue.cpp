...
queue_s *queue_new(void) {
  ...
  if ((queue->data = static_cast<void**>(malloc(QUEUE_INIT_SIZE*sizeof(*(queue->data))))) == NULL) {
    free(queue);
    return NULL;
  }
...
int queue_enqueue(queue_s *queue, void *data) {
  ...
  if (queue->back == queue->front) {
    ...
    void **new_data = static_cast<void**> (realloc(queue->data, new_size*sizeof(*(queue->data))));
    if (new_data == NULL) {
