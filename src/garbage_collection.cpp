...
Result GarbageCollection::moveValidDataToNewArea(AreaPos srcArea, AreaPos dstArea, SummaryEntry* summary){
	...
	, srcArea, dev->areaMgmt.getPos(srcArea), dstArea, dev->areaMgmt.getPos(dstArea));
...
Result GarbageCollection::collectGarbage(AreaType targetType){
	...
	while(1){
		...
		if(srcAreaContainsData){
			...
			//while(getchar() == EOF);
			if(r != Result::ok){
				PAFFS_DBG_S(PAFFS_TRACE_ERROR, "Could not copy valid pages from area %u to %u!", deletion_target, dev->areaMgmt.getActiveArea(AreaType::garbageBuffer));
