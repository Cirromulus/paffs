...
struct InodePool : InodePoolBase
{
    ...
    getExistingInode(InodeNo no, SmartInodePtr& target) override
    {
        ...
        PAFFS_DBG_S(PAFFS_TRACE_BUFFERS, "Increased Refcount of %" PRIinodeno " to %" PRIu8 "", no, it->second.second);
    ...
    removeInodeReference(InodeNo no) override
    {
        ...
        it->second.second--;  // Inode refcount
        PAFFS_DBG_S(PAFFS_TRACE_BUFFERS, "Decreased Refcount of %" PRIinodeno " to %" PRIu8 "", no, it->second.second);
        if (it->second.second == 0)
        {
