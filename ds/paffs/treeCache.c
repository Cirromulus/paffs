...
PAFFS_RESULT addNewCacheNodeWithPossibleFlush(p_dev* dev, treeCacheNode** newTcn){
	...
	if(r != PAFFS_LOWMEM)
		return r;
	//Ok, we have to flush the complete cache now
	flushTreeCache();
	return addNewCacheNode(newTcn);
}
...
PAFFS_RESULT getTreeNodeAtIndexFrom(p_dev* dev, unsigned char index,
	...
	if(target != NULL){
		*child = target;
		PAFFS_DBG(PAFFS_TRACE_CACHE, "Cache hit, found target %p (position %d)", target, target - cache);
	...
	PAFFS_DBG(PAFFS_TRACE_CACHE, "Cache Miss, loaded target %p (position %d)", target, target - cache);
